...
object ReconActors {
  ...
  private[this] class FileTransformerWorker[T, V](engine: ReconEngine[T, V]) 
    ...
    override def receive = {
      case Transform(f, c) => 
        self.reply(engine.fromSource((f, c.asInstanceOf[ReconSource[T]])))
    }
  }
  ...
  private[this] class Processor[T, V](engine: ReconEngine[T, V])
    ...
    override def receive = {
      case ids: List[_] =>
        self.reply(Reconciled(engine.reconcile(ids.asInstanceOf[List[String]], matchHeadAsSumOfRest).seq))
    }
  }
